const { emptyValue, formatDate, getData, getValue, getMoney, getNumber, formatNumber, formatMoney } = require("../../lib/utilities");
const { logoBase64 } = require("../../lib/imgBase64");

const report = async function(data) {
  const title = "Inventario de bodega";
  const subject = "Inventario de bodega";
  const keywords = "inventario, almacen";
  ...
  /** Details */
  let details_widths = [20, 50, 60, 40, "*", 80, 90];
  ...
  let details_print = [
    [
      ...
      { text: `Inventario`, fillColor: "#CCCCCC", alignment: "center" },
  ...
  details.map((item, i) => {
    const item_print = [
      ...
      { text: `${getNumber(item, "stock", "")}`, alignment: "right", maxHeight: 12 },
      { text: `${getValue(item, "unity", "")}`, alignment: "center", maxHeight: 12 },
      { text: `${getValue(item, "description", "")}`, alignment: "left", maxHeight: 12 },
      { text: `${getNumber(item, "quantity", "")}`, alignment: "right" },
      { text: `${getMoney(item, "cost", "")}`, alignment: "right" }
    ];
        ...
        table: {
          ...
          body: [
            [
              ...
              { text: `Inventario`, alignment: "left" },
              { text: `${formatDate(master.date_of, "MMMM d, yyyy")}`, alignment: "center" }
            ],
            [
              { text: `Autorizo`, alignment: "left" },
              { text: `${getData(master, "aprobo", "").toUpperCase()}`, alignment: "left" },
              { text: `Total`, alignment: "left" },
              { text: `${getMoney(master, "cost", "")}`, alignment: "right" }
            ],
            [
              { text: `Realizo`, alignment: "left" },
              { text: `${getData(master, "realizo", "").toUpperCase()}`, alignment: "left" },
      ...
      },
      {
        ...
        table: {
          ...
          body: [
            ...
            [
              { text: `Realizo: ${getData(master, "realizo", "")}`, alignment: "center", maxHeight: 10 },
              { text: `Reviso: ${getData(master, "reviso", "")}`, alignment: "center", maxHeight: 10 },
              { text: `Autorizo: ${getData(master, "aprobo", "")}`, alignment: "center", maxHeight: 10 }
            ]
          ]
        },
        layout: tableLayout
